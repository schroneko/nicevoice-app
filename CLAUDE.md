...
- Voxtral Realtime（Mistral AI、WebSocket リアルタイム音声認識、開発者タブで切替）
...
- Voxtral Realtime（Mistral AI、開発者タブで切替可能）
...
- Voxtral Realtime（Mistral AI）: WebSocket (`wss://api.mistral.ai/v1/audio/transcriptions/realtime`) 経由のリアルタイム文字起こし

Voxtral を選択すると 1Password CLI (`op item get "MISTRAL_API_KEY"`) で API キーを自動取得する。API キーはメモリ上のみ保持し、永続化しない。

### 関連ファイル

- `VoxtralRealtimeService.swift`: WebSocket 接続、PCM 16kHz/16bit/mono への変換、音声ストリーミング
