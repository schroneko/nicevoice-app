...
voxmlx と qwen3asr の Python サーバーは WebSocket プロトコル (OpenAI Realtime API 互換) が同一だが、コードは完全に独立している。新しい ASR モデルを追加するたびに FastAPI + WebSocket のボイラープレートを書き直す状態。共通の WebSocket サーバーフレームワークを作り、モデルごとにアダプター (feed_audio / finish 等) だけ実装する形にすると拡張しやすい。Swift 側 (`LocalASRService`, `LocalServerManager`) は既にパラメータ切替で共通化済み。
...
Phase 2 で英語 UI を予定。参考: VoiceOS.app (Electron) は 60 以上の .lproj を持ち、af/am/ar/bg/bn/ca/cs/da/de/el/en/es/et/fa/fi/fil/fr/gu/he/hi/hr/hu/id/it/ja/kn/ko/lt/lv/ml/mr/ms/nb/nl/pl/pt_BR/pt_PT/ro/ru/sk/sl/sr/sv/sw/ta/te/th/tr/uk/ur/vi/zh_CN/zh_TW に対応している。NiceVoice は String Catalog で段階的に対応言語を増やす方針。
