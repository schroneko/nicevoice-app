...
NICEVOICE_DIR="$(pwd)"
CASK_FILE="${NICEVOICE_DIR}/Casks/nicevoice.rb"
...
sed -i '' 's|^  url ".*"|  url "https://github.com/schroneko/homebrew-nicevoice/releases/download/v#{version}/NiceVoice-#{version}.zip"|' "${CASK_FILE}"

echo "Updated: ${CASK_FILE}"

echo "==> Committing Cask changes..."
...
gh release delete "v${VERSION}" --repo schroneko/homebrew-nicevoice --yes 2>/dev/null || true
git push origin --delete "v${VERSION}" 2>/dev/null || true

gh release create "v${VERSION}" "${NICEVOICE_DIR}/${ARCHIVE}" \
    --title "v${VERSION}" \
    --notes "NiceVoice v${VERSION}" \
    --repo schroneko/homebrew-nicevoice

echo ""
echo "Done! Cask updated and release created."
echo ""
...
echo ""
echo "To verify:"
echo "  brew reinstall --cask schroneko/nicevoice/nicevoice"
